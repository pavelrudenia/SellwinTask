...
"""http://127.0.0.1:5000/basket?csrf_token=ImVjZDExOTgzOTE5NzQ5M2YzOWNlZjI4Mjg3MGVmY2NiNzA0MTZhNzIi.YnDP1Q.2zuvpB4hCJYiHd3jCocbxOa-vKk&language=%D0%B4%D0%B8%D0%B2%D0%B0%D0%BD+%7C+%D0%A6%D0%B5%D0%BD%D0%B0+%D0%B7%D0%B0+%D1%88%D1%82.279.0&language2=xx%7C36535047&count=1
&author=pavel"""
@login_required
...
def basket():
    ...
    if request.method == "POST":
        prod = request.args.get("language")
        card = request.args.get('language2')
        count = request.args.get('count')
        if prod and card and count is not None:
            cards = card.split('|')
            card_series, card_number = cards[0], cards[1]
            print(card_series, card_number)

            count = int(count)
            current_discount = dbase.getCardDiscount(card_number, card_series)
            ...
            card = request.form.get('language2')
            cards = card.split('|')
            card_series,card_number = cards[0],cards[1]
            print(card_series,card_number)
            count = request.form.get('count')
...
def order(count, name_prod, price, current_discount, discount, card_number,card_series):
    if request.method == "POST":
        res = dbase.CreateOrder(count, name_prod, price, current_discount,
                                discount, card_number,card_series)
        if res:
            flash("Заказ успешно создан", "success")
            return redirect(url_for('ShowCard', alias=card_number,series = card_series))
        else:
            flash("Ошибка ", "error")
